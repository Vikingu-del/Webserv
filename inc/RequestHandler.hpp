...
private:
    ...
    void handlePostRequest(std::pair<std::string, std::string> &responseBody,
                           std::map<std::string, HTTP::Header> &responseHeaders);
    void handleRemoveRequest(std::pair<std::string, std::string> &responseBody,
                             std::map<std::string, HTTP::Header> &responseHeaders);
    void handleError(std::pair<std::string, std::string> &responseBody, const std::string &errorPath);
    void handleAutoindex(std::pair<std::string, std::string> &responseBody,
                         std::map<std::string, HTTP::Header> &responseHeaders,
