...
HTTP::Response	HTTP::handleRequest(const std::string &request, const ServerConfig &server) {
    try {
    ...
    return HTTP::Response(HTTP::INTERNAL_SERVER_ERROR, HTTP::HTTP_1_1, std::map<std::string, HTTP::Header>(), "");
}