...
std::string		HTTP::Header::serialize() const {
	return this->_key + ": " + this->_value + LINE_END;
}
...
// Constructors
HTTP::Response::Response() : _responseCode(0), _version(HTTP::HTTP_1_1), _headers(), _body("") {};

HTTP::Response::Response(int responseCode, Version version, const std::map<std::string, Header>& headers, const std::string& body)
...
std::string		HTTP::Response::serialize() const {
	std::string response = versionToString(this->_version) + " " + utils::toString(this->_responseCode) + LINE_END;
	for (std::map<std::string, HTTP::Header>::const_iterator it = this->_headers.begin(); it != this->_headers.end(); ++it)
