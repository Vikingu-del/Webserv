...
/*   Updated: 2024/06/30 22:23:48 by eseferi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ServerConfig::setHost(std::string host)
{
	...
	if (inet_pton(AF_INET, host.c_str(), &(sockaddr.sin_addr)) <= 0) {
        // Consider extending this to support IPv6 with AF_INET6 and sockaddr_in6
        throw ServerConfigException("Invalid host syntax or unsupported address: " + host);
    }
...
{
	if ((_listen_fd = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP) )  == -1 ) // Creation of a tcp socket with ipv4
	{
    ...
    if (setsockopt(_listen_fd, SOL_SOCKET, SO_REUSEADDR, &option_value, sizeof(int)) == -1)
        throw ServerConfigException("setsockopt SO_REUSEADDR error");
    // Set TCP_NODELAY
    if (setsockopt(_listen_fd, IPPROTO_TCP, TCP_NODELAY, &option_value, sizeof(int)) == -1) {
        throw ServerConfigException("setsockopt TCP_NODELAY error");
	} memset(&_server_address, 0, sizeof(sockaddr_in));
	_server_address.sin_family = AF_INET;
	_server_address.sin_addr.s_addr = _host;
	_server_address.sin_port = htons(_port);
	if (bind(_listen_fd, (struct sockaddr *) &_server_address, sizeof(sockaddr_in)) == -1)
	{
